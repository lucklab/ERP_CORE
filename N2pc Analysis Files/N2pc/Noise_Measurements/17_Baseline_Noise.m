...
fid = fopen(fullfile(Current_File_Path, 'Baseline_Noise_N2pc.csv'), 'w');
...
for i = 1:length(SUB)
    ...
    ERP = pop_loaderp('filename', [SUB{i} '_N2pc_erp_ar_diff_waves.erp'], 'filepath', Subject_Path); 
...
baseline_noise_table = readtable([Current_File_Path filesep 'Baseline_Noise_N2pc.csv']); 
...
save2pdf([Current_File_Path filesep 'N2pc_Baseline_Noise_Unfiltered.pdf']);
close all
...
%Save baseline noise plots to pdf
save2pdf([Current_File_Path filesep 'N2pc_Baseline_Noise_Filtered.pdf']);
close all

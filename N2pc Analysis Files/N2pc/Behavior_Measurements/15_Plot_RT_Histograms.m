...
for i = 1:length(SUB)
    ...
    EEG = pop_loadset( 'filename', [SUB{i} '_N2pc_shifted_ds_reref_ucbip_hpfilt_ica_corr_cbip_elist.set'], 'filepath', Subject_Path);
 
    %Assign events to left target and right target trial bins using no reaction time filter with Binlister (bin assignments can be reviewed in each subject's N2pc_Eventlist_For_Histo_RTs_Bins.txt file)
    EEG  = pop_binlister( EEG , 'BDF', [Current_File_Path filesep 'BDF_Histo_RTs_N2pc.txt'], 'ExportEL', [Subject_Path SUB{i} '_N2pc_Eventlist_For_Histo_RTs_Bins.txt'], 'IndexEL',  1, 'SendEL2', 'EEG&Text', 'UpdateEEG', 'on', 'Voutput', 'EEG' );

    %Extract reaction time (RT) data for each trial per bin and save to a text file in each subject's folder
    pop_rt2text(EEG, 'filename', [Subject_Path SUB{i} '_N2pc_Histo_RTs.txt'], 'Header', 'on', 'listformat', 'basic');
...
for i = 1:length(SUB)
    ...
    Single_Sub_Data = readtable([Subject_Path SUB{i} '_N2pc_Histo_RTs.txt'],'ReadVariableNames',1,'Delimiter',{'\t'});

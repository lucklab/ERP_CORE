...
%This script loads the epoched EEG data file from Script #5, interpolates bad channels listed in Excel file Interpolate_Channels_LRP.xls, and performs artifact rejection to remove noisy segments
...
%Current_File_Path = /Users/KappenmanLab/ERP_CORE/LRP/EEG_ERP_Processing
...
[ndata1, text1, alldata1] = xlsread([Current_File_Path filesep 'Interpolate_Channels_LRP']);
...
[ndata2, text2, alldata2] = xlsread([Current_File_Path filesep 'AR_Parameters_for_SVT_CRAP_LRP']);
...
[ndata3, text3, alldata3] = xlsread([Current_File_Path filesep 'AR_Parameters_for_MW_CRAP_LRP']);
...
[ndata4, text4, alldata4] = xlsread([Current_File_Path filesep 'AR_Parameters_for_SL_HEOG_LRP']);
...
for i = 1:length(SUB)
    ...
    EEG = pop_loadset( 'filename', [SUB{i} '_LRP_shifted_ds_reref_ucbip_hpfilt_ica_corr_cbip_elist_bins_epoch.set'], 'filepath', Subject_Path);
    [ALLEEG EEG CURRENTSET] = pop_newset(ALLEEG, EEG, 1, 'setname', [SUB{i} '_LRP_shifted_ds_reref_ucbip_hpfilt_ica_corr_cbip_elist_bins_epoch'], 'gui', 'off'); 

    %Interpolate channel(s) specified in Excel file Interpolate_Channels_LRP.xls; any channels without channel locations (e.g., the eye channels) should not be included in the interpolation process and are listed in ignored channels
    ...
    for j = 1:DimensionsOfFile1(1);
        if isequal(SUB{i},num2str(alldata1{j,1}));
           ...
           [ALLEEG EEG CURRENTSET] = pop_newset(ALLEEG, EEG, 2, 'setname', [SUB{i} '_LRP_shifted_ds_reref_ucbip_hpfilt_ica_corr_cbip_elist_bins_epoch_interp'], 'savenew', [Subject_Path SUB{i} '_LRP_shifted_ds_reref_ucbip_hpfilt_ica_corr_cbip_elist_bins_epoch_interp.set'], 'gui', 'off'); 
        end
    end
    ...
    [ALLEEG EEG CURRENTSET] = pop_newset(ALLEEG, EEG, 3, 'setname', [SUB{i} '_LRP_shifted_ds_reref_ucbip_hpfilt_ica_corr_cbip_elist_bins_epoch_interp_SVT'], 'gui', 'off'); 
    ...
    [ALLEEG EEG CURRENTSET] = pop_newset(ALLEEG, EEG, 4, 'setname', [SUB{i} '_LRP_shifted_ds_reref_ucbip_hpfilt_ica_corr_cbip_elist_bins_epoch_interp_SVT_MW'], 'gui', 'off'); 
    ...
    [ALLEEG EEG CURRENTSET] = pop_newset(ALLEEG, EEG, 5, 'setname', [SUB{i} '_LRP_shifted_ds_reref_ucbip_hpfilt_ica_corr_cbip_elist_bins_epoch_interp_SVT_MW_SL'], 'savenew', [Subject_Path SUB{i} '_LRP_shifted_ds_reref_ucbip_hpfilt_ica_corr_cbip_elist_bins_epoch_interp_ar.set'], 'gui', 'off'); 

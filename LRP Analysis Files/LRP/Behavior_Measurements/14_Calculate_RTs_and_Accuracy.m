...
for i = 1:length(SUB)
    ...
    EEG = pop_exporteegeventlist( EEG , 'Filename', [Subject_Path SUB{i} '_LRP_Eventlist_For_RTs.txt']); 

    %Load the semi-continuous ICA-corrected EEG data file outputted from Script #5 in .set EEGLAB file format
    EEG = pop_loadset( 'filename', [SUB{i} '_LRP_shifted_ds_reref_ucbip_hpfilt_ica_corr.set'], 'filepath', Subject_Path);
    
    %Import the eventlist exported above (with epochs marked for artifact rejection) to the semi-continuous EEG data file  
    EEG = pop_importeegeventlist( EEG, [Subject_Path SUB{i} '_LRP_Eventlist_For_RTs.txt'], 'ReplaceEventList', 'on' ); 

    %Assign events to correct and incorrect trial bins for each condition (both before and after artifact rejection) with Binlister; an individual trial may be assigned to more than one bin (bin assignments can be reviewed in each subject's LRP_Eventlist_Bins_RTs.txt file)
    EEG  = pop_binlister( EEG , 'BDF', [Current_File_Path filesep 'BDF_LRP_RTs.txt'], 'ExportEL', [Subject_Path SUB{i} '_LRP_Eventlist_RTs.txt'], 'IndexEL',  1, 'SendEL2', 'EEG&Text', 'UpdateEEG', 'on', 'Voutput', 'EEG' );

    %Extract reaction time (RT) data for each trial in each bin and save to a text file in each subject's folder
    pop_rt2text(EEG, 'filename', [Subject_Path SUB{i} '_RTs_&_ACC_LRP.txt'], 'Header', 'on', 'listformat', 'basic');
...
fid = fopen(fullfile(Current_File_Path, 'RTs_Trial_Counts_&_ACCs_LRP.csv'), 'w');
...
for i = 1:length(SUB)
    ...
    RT_table = readtable([Subject_Path SUB{i} '_RTs_&_ACC_LRP.txt'],'delimiter','tab'); 
    
    %Calculate the mean reaction time for each bin (excluding trials marked for artifacts)
    Mean_RT = nanmean(RT_table{:,1:18},1);
    
    %Calculate the trial counts for each bin
    Trial_Counts = sum(~isnan(RT_table{:,1:18}),1);
    
    %Calculate the accuracy for the bin(s) of interest (e.g., all trials, prior to artifact rejection) 
    LeftAccuracy = Trial_Counts(7) / (Trial_Counts(7) + Trial_Counts(13)) * 100;  
    RightAccuracy = Trial_Counts(8) / (Trial_Counts(8) + Trial_Counts(14)) * 100;  

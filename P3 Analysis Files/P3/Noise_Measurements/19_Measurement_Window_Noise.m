...
fid = fopen(fullfile(Current_File_Path, 'MeasurementWindow_Noise_P3.csv'), 'w');
fprintf(fid, 'SubID, Rare, Frequentsi, Rare-Frequent, Rare Filtered, Frequentsi Filtered, Rare-Frequent Filtered\n');

%***********************************************************************************************************************************************

%Set measurement time window in milliseconds (e.g., 300 to 600 ms)
timewindow = [300 600];
...
for i = 1:length(SUB)
    ...
    ERP = pop_loaderp('filename', [SUB{i} '_P3_erp_ar_diff_waves_Odd_minus_Even.erp'], 'filepath', Subject_Path); 
    ...
    ERP = pop_loaderp('filename', [SUB{i} '_P3_erp_ar_diff_waves_Odd_minus_Even_lpfilt.erp'], 'filepath', Subject_Path); 
...
meas_noise_table = readtable([Current_File_Path filesep 'MeasurementWindow_Noise_P3.csv']);
...
save2pdf([Current_File_Path filesep 'P3_MeasurementWindow_Noise_Unfiltered.pdf']);
close all
...
%Save measurement window noise plots to pdf
save2pdf([Current_File_Path filesep 'P3_MeasurementWindow_Noise_Filtered.pdf']);
close all

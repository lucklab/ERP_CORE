...
for i = 1:length(SUB)
    ...
    EEG = pop_loadset( 'filename', [SUB{i} '_P3_shifted_ds_reref_ucbip_hpfilt_ica_corr_cbip_elist.set'], 'filepath', Subject_Path);
 
    %Assign events to rare and frequent trial bins using no reaction time filter with Binlister (bin assignments can be reviewed in each subject's P3_Eventlist_For_Histo_RTs_Bins.txt file)
    EEG  = pop_binlister( EEG , 'BDF', [Current_File_Path filesep 'BDF_Histo_RTs_P3.txt'], 'ExportEL', [Subject_Path SUB{i} '_P3_Eventlist_For_Histo_RTs_Bins.txt'], 'IndexEL',  1, 'SendEL2', 'EEG&Text', 'UpdateEEG', 'on', 'Voutput', 'EEG' );

    %Extract reaction time (RT) data for each trial per bin and save to a text file in each subject's folder
    pop_rt2text(EEG, 'filename', [Subject_Path SUB{i} '_P3_Histo_RTs.txt'], 'Header', 'on', 'listformat', 'basic');
...
for i = 1:length(SUB)
    ...
    Single_Sub_Data = readtable([Subject_Path SUB{i} '_P3_Histo_RTs.txt'],'ReadVariableNames',1,'Delimiter',{'\t'});
    
    Single_Sub_Rare_RTs = Single_Sub_Data{:,1};
    Single_Sub_Frequent_RTs = Single_Sub_Data{:,2};
    
    %Combine reaction times from all subjects
    All_Sub_Rare_RTs = [All_Sub_Rare_RTs; Single_Sub_Rare_RTs];
    All_Sub_Frequent_RTs = [All_Sub_Frequent_RTs; Single_Sub_Frequent_RTs];
...
Probability_Rare_Per_RT_Bin = histcounts(All_Sub_Rare_RTs,xrange,'Normalization','probability');
Probability_Frequent_Per_RT_Bin = histcounts(All_Sub_Frequent_RTs,xrange,'Normalization','probability');

bar(bincenters, Probability_Rare_Per_RT_Bin, w1, 'FaceColor', [0.2 0.2 0.5]);
hold on;
bar(bincenters, Probability_Frequent_Per_RT_Bin, w2, 'FaceColor',[0 0.7 0.7]);
hold off;
...
save2pdf([Current_File_Path filesep 'RT_Probability_Histogram_Rare_&_Frequent.pdf']);
close all

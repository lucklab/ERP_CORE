...
for i = 1:length(SUB)
    ...
    for condition = Conditions
        ...
        ERP = pop_savemyerp( ERP, 'erpname', [SUB{i} '_P3_erp_ar_' condition{:}], 'filename', [Subject_Path SUB{i} '_P3_erp_ar_' condition{:} '.erp']);
        
        %Create a difference ERP waveform using only the trials from the specified condition (e.g., odd-numbered trials)
        ERP = pop_binoperator( ERP, [DIR filesep 'EEG_ERP_Processing' filesep 'P3_Diff_Wave.txt']);
        ERP = pop_savemyerp(ERP, 'erpname', [SUB{i} '_P3_erp_ar_diff_waves_' condition{:}], 'filename', [DIR filesep SUB{i} filesep SUB{i} '_P3_erp_ar_diff_waves_' condition{:} '.erp']);
...
for i = 1:length(SUB)
    ...
    [ERP ALLERP] = pop_loaderp( 'filename', {[SUB{i} '_P3_erp_ar_diff_waves_ODD.erp'], [SUB{i} '_P3_erp_ar_diff_waves_Even.erp']}, 'filepath', Subject_Path);                                                                                                     
	...
	ERP = pop_binoperator( ERP, {  'b7 = ((b1 - b4)/2) Label Odd Rare Wave - Even Rare Wave', 'b8 = ((b2 - b5)/2) Label Odd Frequent Wave - Even Frequent Wave', 'b9 = ((b3 - b6)/2) Label Odd Diff Wave - Even Diff Wave'});
    ERP = pop_savemyerp(ERP, 'erpname', [SUB{i} '_P3_erp_ar_diff_waves_Odd_minus_Even'], 'filename', [SUB{i} '_P3_erp_ar_diff_waves_Odd_minus_Even.erp'], 'filepath', [DIR filesep SUB{i} filesep]);   
    ...
    ERP = pop_savemyerp(ERP, 'erpname', [SUB{i} '_P3_erp_ar_diff_waves_Odd_minus_Even_lpfilt'], 'filename', [SUB{i} '_P3_erp_ar_diff_waves_Odd_minus_Even_lpfilt.erp'], 'filepath', Subject_Path, 'Warning', 'off');
...
for i = 1:length(SUB)  
    ...
    [ERP ALLERP] = pop_loaderp( 'filename', [SUB{i} '_P3_erp_ar_diff_waves_Odd_minus_Even_lpfilt.erp'], 'filepath', [DIR filesep SUB{i} filesep]);      
end

%Append low-pass filtered ERP waveforms from all subjects 
ERP = pop_appenderp( ALLERP , 'Erpsets', 1:length(SUB), 'Prefixes', SUB);
ERP = pop_savemyerp(ERP, 'erpname', 'All_Sub_Appended_Odd_minus_Even_P3_lpfilt', 'filename', 'All_Sub_Appended_Odd_minus_Even_P3_lpfilt.erp', 'filepath', Current_File_Path, 'Warning', 'off');
...
%Plot individual subject plus-minus waveforms for rare trials (bin 7)
bin = 7;
...
save2pdf([Current_File_Path filesep 'P3_PlusMinus_Waveforms_Rare.pdf']);
close all

%Plot individual subject plus-minus waveforms for frequent trials (bin 88)
bin = 8;
ERP = pop_ploterps( ERP,  bin:9:ERP.nbin,  chan , 'Box', [ 1 1], 'Maximize', 'on', 'Style', 'Classic', 'xscale', xscale, 'yscale', yscale, 'linespec', linecolors);
save2pdf([Current_File_Path filesep 'P3_PlusMinus_Waveforms_Frequentsi.pdf']);
close all

%Plot individual subject plus-minus waveforms for rare minus frequent trials (bin 9)
bin = 9;
ERP = pop_ploterps( ERP,  bin:9:ERP.nbin,  chan , 'Box', [ 1 1], 'Maximize', 'on', 'Style', 'Classic', 'xscale', xscale, 'yscale', yscale, 'linespec', linecolors);
save2pdf([Current_File_Path filesep 'P3_PlusMinus_Waveforms_Rare-Frequent.pdf']);
close all
...
%Extract data from specified bin (e.g., bin 9 - rare minus frequent trials)
bin = 9;
...
save2pdf([Current_File_Path filesep 'P3_PlusMinus_SD.pdf']);
close all;

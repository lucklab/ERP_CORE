...
fid = fopen(fullfile(Current_File_Path, 'Baseline_Noise_LRP.csv'), 'w');
fprintf(fid, 'SubID, Contra, Ipsi, Contra-Ipsi, Contra Filtered, Ipsi Filtered, Contra-Ipsi Filtered\n');

%*************************************************************************************************************************************

%Set baseline period time window in milliseconds (e.g., -800 to -600 ms)
timewindow = [-800 -600];
...
for i = 1:length(SUB)
    ...
    ERP = pop_loaderp('filename', [SUB{i} '_LRP_erp_ar_diff_waves.erp'], 'filepath', Subject_Path); 
...
baseline_noise_table = readtable([Current_File_Path filesep 'Baseline_Noise_LRP.csv']); 
...
save2pdf([Current_File_Path filesep 'LRP_Baseline_Noise_Unfiltered.pdf']);
close all
...
%Save baseline noise plots to pdf
save2pdf([Current_File_Path filesep 'LRP_Baseline_Noise_Filtered.pdf']);
close all

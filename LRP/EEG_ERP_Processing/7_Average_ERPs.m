...
%and saves the information to a .csv file in each subject's data folder, calculates ERP difference waveforms between conditions (collapsing conditions across left and right hemisphere electrode sites), 
%and creates low-pass filtered versions of the ERP waveforms.
...
for i = 1:length(SUB)
    ...
    ERP = pop_savemyerp( ERP, 'erpname', [SUB{i} '_LRP_erp_ar'], 'filename', [Subject_Path SUB{i} '_LRP_erp_ar.erp']);
    ...
    ERP = pop_savemyerp( ERP, 'erpname', [SUB{i} '_LRP_erp_ar_lpfilt'], 'filename', [Subject_Path SUB{i} '_LRP_erp_ar_lpfilt.erp']);
    ...
    fid = fopen([DIR filesep SUB{i} filesep SUB{i} '_AR_Percentages_LRP.csv'], 'w');
...
for i = 1:length(SUB)
    ...
    ERP = pop_loaderp('filename', [SUB{i} '_LRP_erp_ar.erp'], 'filepath', Subject_Path);                                                                                                                                                                                                                                                                                                                                       

    %Create ERP difference waveforms between conditions
    ERP = pop_binoperator( ERP, [Current_File_Path filesep 'LRP_Diff_Wave.txt']);
    ERP = pop_savemyerp(ERP, 'erpname', [SUB{i} '_LRP_erp_ar_diff_waves'], 'filename', [Subject_Path SUB{i} '_LRP_erp_ar_diff_waves.erp']);
    ...
    ERP = pop_savemyerp( ERP, 'erpname', [SUB{i} '_LRP_erp_ar_diff_waves_lpfilt'], 'filename', [Subject_Path SUB{i} '_LRP_erp_ar_diff_waves_lpfilt.erp']);

...
for i = 1:length(SUB)
    ...
    EEG = pop_loadset( 'filename', [SUB{i} '_LRP.set'], 'filepath', Subject_Path);
    [ALLEEG EEG CURRENTSET] = pop_newset(ALLEEG, EEG, 1, 'setname', [SUB{i} '_LRP'], 'gui', 'off'); 
    ...
    EEG  = pop_erplabShiftEventCodes( EEG , 'DisplayEEG', 0, 'DisplayFeedback', 'summary', 'Eventcodes', [11 12 21 22], 'Rounding', 'earlier', 'Timeshift',  26 );
    [ALLEEG EEG CURRENTSET] = pop_newset(ALLEEG, EEG, 2,'setname', [SUB{i} '_LRP_shifted'],'savenew',[Subject_Path SUB{i} '_LRP_shifted.set'] ,'gui','off'); 
    ...
    [ALLEEG EEG CURRENTSET] = pop_newset(ALLEEG, EEG, 3,'setname',[SUB{i} '_LRP_shifted_ds'],'savenew',[Subject_Path SUB{i} '_LRP_shifted_ds.set'] ,'gui','off'); 
    ...
    EEG = pop_eegchanoperator( EEG, [Current_File_Path filesep 'Rereference_Add_Uncorrected_Bipolars_LRP.txt']);
    ...
    [ALLEEG EEG CURRENTSET] = pop_newset(ALLEEG, EEG, 4, 'setname', [SUB{i} '_LRP_shifted_ds_reref_ucbip'], 'savenew', [Subject_Path SUB{i} '_LRP_shifted_ds_reref_ucbip.set'], 'gui', 'off');
    ...
    [ALLEEG EEG CURRENTSET] = pop_newset(ALLEEG, EEG, 5, 'setname', [SUB{i} '_LRP_shifted_ds_reref_ucbip_hpfilt'], 'savenew', [Subject_Path SUB{i} '_LRP_shifted_ds_reref_ucbip_hpfilt.set'], 'gui', 'off');

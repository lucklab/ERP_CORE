...
for i = 1:length(SUB)
    ...
    EEG = pop_exporteegeventlist( EEG , 'Filename', [Subject_Path SUB{i} '_N400_Eventlist_For_RTs.txt']); 

    %Load the semi-continuous ICA-corrected EEG data file outputted from Script #5 in .set EEGLAB file format
    EEG = pop_loadset( 'filename', [SUB{i} '_N400_shifted_ds_reref_ucbip_hpfilt_ica_corr.set'], 'filepath', Subject_Path);
    
    %Import the eventlist exported above (with epochs marked for artifact rejection) to the semi-continuous EEG data file  
    EEG = pop_importeegeventlist( EEG, [Subject_Path SUB{i} '_N400_Eventlist_For_RTs.txt'], 'ReplaceEventList', 'on' ); 

    %Assign events to correct and incorrect trial bins for each condition (both before and after artifact rejection) with Binlister; an individual trial may be assigned to more than one bin (bin assignments can be reviewed in each subject's N400_Eventlist_Bins_RTs.txt file)
    EEG  = pop_binlister( EEG , 'BDF', [Current_File_Path filesep 'BDF_N400_RTs.txt'], 'ExportEL', [Subject_Path SUB{i} '_N400_Eventlist_RTs.txt'], 'IndexEL',  1, 'SendEL2', 'EEG&Text', 'UpdateEEG', 'on', 'Voutput', 'EEG' );

    %Extract reaction time (RT) data for each trial in each bin and save to a text file in each subject's folder
    pop_rt2text(EEG, 'filename', [Subject_Path SUB{i} '_RTs_&_ACC_N400.txt'], 'Header', 'on', 'listformat', 'basic');
...
fid = fopen(fullfile(Current_File_Path, 'RTs_Trial_Counts_&_ACCs_N400.csv'), 'w');
fprintf(fid, ['SubID, Unrelated All Correct RT (After AR), Related All Correct RT (After AR),'...
	'Unrelated All Correct Trial Count (After AR), Related All Correct Trial Count (After AR),'...
	'Unrelated Accuracy (Before AR), Related Accuracy (Before AR), Total Accuracy (Before AR)\n']); 
...
for i = 1:length(SUB)
    ...
    RT_table = readtable([Subject_Path SUB{i} '_RTs_&_ACC_N400.txt'],'delimiter','tab'); 
    ...
    UnrelatedAccuracy = Trial_Counts(5) / (Trial_Counts(5) + Trial_Counts(7)) * 100;  
    RelatedAccuracy = Trial_Counts(6) / (Trial_Counts(6) + Trial_Counts(8)) * 100;  
    TotalAccuracy = (UnrelatedAccuracy + RelatedAccuracy) / 2;  
    
    %Save the mean reaction time (RT), trial counts, and accuracy information for bin(s) of interest to a .csv file 
    fprintf(fid, '%s,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f\n', SUB{i}, Mean_RT(1:2), Trial_Counts(1:2), UnrelatedAccuracy, RelatedAccuracy, TotalAccuracy);

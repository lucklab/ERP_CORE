...
fid = fopen(fullfile(Current_File_Path, 'MeasurementWindow_Noise_N170.csv'), 'w');
fprintf(fid, 'SubID, Faces, Cars, Faces-Cars, Faces Filtered, Cars Filtered, Faces-Cars Filtered\n');

%***********************************************************************************************************************************************

%Set measurement time window in milliseconds (e.g., 110 to 150 ms)
timewindow = [110 150];
...
for i = 1:length(SUB)
    ...
    ERP = pop_loaderp('filename', [SUB{i} '_N170_erp_ar_diff_waves_Odd_minus_Even.erp'], 'filepath', Subject_Path); 
    ...
    ERP = pop_loaderp('filename', [SUB{i} '_N170_erp_ar_diff_waves_Odd_minus_Even_lpfilt.erp'], 'filepath', Subject_Path); 
...
meas_noise_table = readtable([Current_File_Path filesep 'MeasurementWindow_Noise_N170.csv']);
...
legend({'Faces','Cars','Faces minus Cars'});
set(gca,'fontsize',12);

%Save measurement window noise plots to pdf
save2pdf([Current_File_Path filesep 'N170_MeasurementWindow_Noise_Unfiltered.pdf']);
close all
...
%Save measurement window noise plots to pdf
save2pdf([Current_File_Path filesep 'N170_MeasurementWindow_Noise_Filtered.pdf']);
close all

...
%This script loads the outputted semi-continuous EEG data file containing the ICA weights from Script #3, loads the list of ICA component(s) from the ICA_Components_N170.xlsx Excel file, and removes the component(s) from the EEG.
...
for i = 1:length(SUB)
    ...
    EEG = pop_loadset('filename',[SUB{i} '_N170_shifted_ds_reref_ucbip_hpfilt_ica_weighted.set'],'filepath', Subject_Path);
    [ALLEEG EEG CURRENTSET] = pop_newset(ALLEEG, EEG, 1,'setname',[SUB{i} '_N170_shifted_ds_reref_ucbip_hpfilt_ica_weighted'], 'gui','off'); 

    %Load list of ICA component(s) corresponding to ocular artifacts from Excel file ICA_Components_N170.xlsx
    [ndata, text, alldata] = xlsread([Current_File_Path filesep 'ICA_Components_N170']); 
    ...
    [ALLEEG EEG CURRENTSET] = pop_newset(ALLEEG, EEG, 2,'setname',[SUB{i} '_N170_shifted_ds_reref_ucbip_hpfilt_ica_corr'],'savenew', [Subject_Path SUB{i} '_N170_shifted_ds_reref_ucbip_hpfilt_ica_corr.set'],'gui','off'); 
    ...
    EEG = pop_eegchanoperator( EEG, [Current_File_Path filesep 'Add_Corrected_Bipolars_N170.txt']);
    ...
    [ALLEEG EEG CURRENTSET] = pop_newset(ALLEEG, EEG, 3, 'setname', [SUB{i} '_N170_shifted_ds_reref_ucbip_hpfilt_ica_corr_cbip'], 'savenew', [Subject_Path SUB{i} '_N170_shifted_ds_reref_ucbip_hpfilt_ica_corr_cbip.set'], 'gui', 'off'); 

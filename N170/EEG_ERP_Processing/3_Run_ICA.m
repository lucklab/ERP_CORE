...
% % As a result, the topographic maps of the ICA weights and the excel spreadsheet (ICA_Components_N170.xlsx) containing the list of ICA components to be removed for each subject included in this package 
...
%
% % If you wish to re-compute ICA weights on the ERP CORE data, you will need to disregard the information in ICA_Components_N170.xslx and evaluate the scalp topography and 
...
%     EEG = pop_loadset( 'filename', [SUB{i} '_N170_shifted_ds_reref_ucbip_hpfilt_ica_prep2.set'], 'filepath', Subject_Path);
%     [ALLEEG EEG CURRENTSET] = pop_newset(ALLEEG, EEG, 1, 'setname', [SUB{i} '_N170_shifted_ds_reref_ucbip_hpfilt_ica_prep2'], 'gui', 'off');   
% 
...
%     [ALLEEG EEG CURRENTSET] = pop_newset(ALLEEG, EEG, 2, 'setname', [SUB{i} '_N170_shifted_ds_reref_ucbip_hpfilt_ica_prep2_weighted'], 'savenew', [Subject_Path SUB{i} '_N170_shifted_ds_reref_ucbip_hpfilt_ica_prep2_weighted.set'], 'gui', 'off');
% 
%     %Load the continuous EEG data file outputted from Script #1 in .set EEGLAB file format
%     EEG = pop_loadset( 'filename', [SUB{i} '_N170_shifted_ds_reref_ucbip_hpfilt.set'], 'filepath', Subject_Path);
%     [ALLEEG EEG CURRENTSET] = pop_newset(ALLEEG, EEG, 3, 'setname', [SUB{i} '_N170_shifted_ds_reref_ucbip_hpfilt'], 'gui', 'off'); 
% 
...
%     [ALLEEG EEG CURRENTSET] = pop_newset(ALLEEG, EEG, 4, 'setname', [SUB{i} '_N170_shifted_ds_reref_ucbip_hpfilt_ica_weighted'], 'savenew', [Subject_Path SUB{i} '_N170_shifted_ds_reref_ucbip_hpfilt_ica_weighted.set'], 'gui', 'off');
% 
%     %Save a pdf of the topographic maps of the ICA weights for later review
%     set(groot,'DefaultFigureColormap',jet)
%     pop_topoplot(EEG, 0, [1:31],[SUB{i} '_N170_shifted_ds_reref_ucbip_hpfilt_ica_prep2_weighted'], [6 6] ,0,'electrodes','on');
%     save2pdf([Subject_Path 'graphs' filesep SUB{i} '_N170_ICA_Weights.pdf']);
%     close all
%     
% %End subject loop
% end

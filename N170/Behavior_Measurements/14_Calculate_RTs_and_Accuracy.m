...
%Current_File_Path = /Users/KappenmanLab/ERP_CORE/N170/Behavior_Measurements
...
for i = 1:length(SUB)
    ...
    EEG = pop_loadset( 'filename', [SUB{i} '_N170_shifted_ds_reref_ucbip_hpfilt_ica_corr_cbip_elist_bins_epoch_interp_ar.set'], 'filepath', Subject_Path);
        
    %Export an eventlist containing a record of all event codes with epochs marked for artifact rejection flagged 
    EEG = pop_exporteegeventlist( EEG , 'Filename', [Subject_Path SUB{i} '_N170_Eventlist_For_RTs.txt']); 

    %Load the semi-continuous ICA-corrected EEG data file outputted from Script #5 in .set EEGLAB file format
    EEG = pop_loadset( 'filename', [SUB{i} '_N170_shifted_ds_reref_ucbip_hpfilt_ica_corr.set'], 'filepath', Subject_Path);
    
    %Import the eventlist exported above (with epochs marked for artifact rejection) to the semi-continuous EEG data file  
    EEG = pop_importeegeventlist( EEG, [Subject_Path SUB{i} '_N170_Eventlist_For_RTs.txt'], 'ReplaceEventList', 'on' ); 
    
    %Assign events to correct and incorrect trial bins for each condition (both before and after artifact rejection) with Binlister; an individual trial may be assigned to more than one bin (bin assignments can be reviewed in each subject's N170_Eventlist_Bins_RTs.txt file)
    EEG  = pop_binlister( EEG , 'BDF', [Current_File_Path filesep 'BDF_N170_RTs.txt'], 'ExportEL', [Subject_Path SUB{i} '_N170_Eventlist_RTs.txt'], 'IndexEL',  1, 'SendEL2', 'EEG&Text', 'UpdateEEG', 'on', 'Voutput', 'EEG' );

    %Extract reaction time (RT) data for each trial in each bin and save to a text file in each subject's folder
    pop_rt2text(EEG, 'filename', [Subject_Path SUB{i} '_RTs_&_ACC_N170.txt'], 'Header', 'on', 'listformat', 'basic');
...
fid = fopen(fullfile(Current_File_Path, 'RTs_Trial_Counts_&_ACCs_N170.csv'), 'w');
fprintf(fid, ['SubID, Faces Correct RT (After AR), Cars Correct RT (After AR), Scrambled Faces Correct RT (After AR), Scrambled Cars Correct RT (After AR),'...
	 'Faces Correct Trial Count (After AR), Cars Correct Trial Count (After AR), Scrambled Faces Correct Trial Count (After AR), Scrambled Cars Correct Trial Count (After AR),'...
	 'Faces Accuracy (Before AR), Cars Accuracy (Before AR), Scrambled Faces Accuracy (Before AR), Scrambled Cars Accuracy (Before AR), Total Accuracy (Before AR)\n']);
...
for i = 1:length(SUB)
    ...
    RT_table = readtable([Subject_Path SUB{i} '_RTs_&_ACC_N170.txt'],'delimiter','tab'); 
    
    %Calculate the mean reaction time for each bin (excluding trials marked for artifacts)
    Mean_RT = nanmean(RT_table{:,1:16},1);
    
    %Calculate the trial counts per bin
    Trial_Counts = sum(~isnan(RT_table{:,1:16}),1);
    
    %Calculate the accuracy for the bin(s) of interest (e.g., all trials, prior to artifact rejection) 
    FacesAccuracy = Trial_Counts(9) / (Trial_Counts(9) + Trial_Counts(13)) * 100;  
    CarsAccuracy = Trial_Counts(10) / (Trial_Counts(10) + Trial_Counts(14)) * 100;  
    ScrambledFacesAccuracy = Trial_Counts(11) / (Trial_Counts(11) + Trial_Counts(15)) * 100;  
    ScrambledCarsAccuracy = Trial_Counts(12) / (Trial_Counts(12) + Trial_Counts(16)) * 100;  
    
    TotalAccuracy = mean([FacesAccuracy CarsAccuracy ScrambledFacesAccuracy ScrambledCarsAccuracy]); 
    
    %Save the mean reaction time (RT), trial counts, and accuracy information for bin(s) of interest to a .csv file 
    fprintf(fid, '%s,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f\n', SUB{i}, Mean_RT(1:4), Trial_Counts(1:4), ...
        FacesAccuracy, CarsAccuracy, ScrambledFacesAccuracy, ScrambledCarsAccuracy, TotalAccuracy);

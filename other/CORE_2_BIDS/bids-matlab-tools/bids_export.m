...
if isempty(opt.eInfo)
    ...
    if isfield(EEG.event, 'duration')       opt.eInfo(end+1,:) = { 'duration'      'duration' }; 
    elseif ~isempty(opt.trialtype)          opt.eInfo(end+1,:) = { 'duration'      'xxxx' }; end
    ...
    if isfield(EEG.event, 'latency'),       opt.eInfo(end+1,:) = { 'sample'        'latency'}; end
end

% reorder fields so it matches BIDS
% fieldOrder = { 'onset' 'duration' 'sample' 'trial_type' 'response_time' 'stim_file' 'value' 'HED' };
fieldOrder = { 'onset' 'duration' 'sample' 'trial_type' 'stim_file' 'value'};
        ...
        if strcmpi(tmpField, 'n/a')
            ...
            switch opt.eInfo{iField,1}
                ...
                case 'onset'
                    ...
                    fprintf(fid, '%1.4f', onset);

                case 'duration'
                    
                    duration = [];
                    if isfield(EEG.event, tmpField) && ~isempty(EEG.event(iEvent).(tmpField))
                        duration = num2str(EEG.event(iEvent).(tmpField), '%1.1f');
                    elseif ~isempty(opt.trialtype)
                        ...
                        if ~isempty(eventVal)
                            ...
                            if ~isempty(indTrial)
                                duration = opt.trialtype{indTrial,3};
                                duration = num2str(duration, '%1.1f');
                            end
                        end
                    end 
                ...
                case 'sample'           
                    if isfield(EEG.event, tmpField)
                       sample = num2str(EEG.event(iEvent).(tmpField)); %modified by wz
...
if ~isfield(tInfo,'EEGChannelCount')

...
fid = fopen(fullfile(Current_File_Path, 'Baseline_Noise_ERN.csv'), 'w');
fprintf(fid, 'SubID, Incorrect, Correct, Incorrect-Correct, Incorrect Filtered, Correct Filtered, Incorrect-Correct Filtered\n');

%*************************************************************************************************************************************

%Set baseline period time window in milliseconds (e.g., -400 to -200 ms)
timewindow = [-400 -200];
...
for i = 1:length(SUB)
    ...
    ERP = pop_loaderp('filename', [SUB{i} '_ERN_erp_ar_diff_waves.erp'], 'filepath', Subject_Path); 
...
baseline_noise_table = readtable([Current_File_Path filesep 'Baseline_Noise_ERN.csv']); 
...
save2pdf([Current_File_Path filesep 'ERN_Baseline_Noise_Unfiltered.pdf']);
close all
...
%Save baseline noise plots to pdf
save2pdf([Current_File_Path filesep 'ERN_Baseline_Noise_Filtered.pdf']);
close all

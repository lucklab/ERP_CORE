...
for i = 1:length(SUB)
    ...
    for condition = Conditions
        ...
        ERP = pop_savemyerp( ERP, 'erpname', [SUB{i} '_ERN_erp_ar_' condition{:}], 'filename', [Subject_Path SUB{i} '_ERN_erp_ar_' condition{:} '.erp']);
        
        %Create a difference ERP waveform using only the trials from the specified condition (e.g., odd-numbered trials)
        ERP = pop_binoperator( ERP, [DIR filesep 'EEG_ERP_Processing' filesep 'ERN_Diff_Wave.txt']);
        ERP = pop_savemyerp(ERP, 'erpname', [SUB{i} '_ERN_erp_ar_diff_waves_' condition{:}], 'filename', [DIR filesep SUB{i} filesep SUB{i} '_ERN_erp_ar_diff_waves_' condition{:} '.erp']);
...
for i = 1:length(SUB)
    ...
    [ERP ALLERP] = pop_loaderp( 'filename', {[SUB{i} '_ERN_erp_ar_diff_waves_ODD.erp'], [SUB{i} '_ERN_erp_ar_diff_waves_Even.erp']}, 'filepath', Subject_Path);                                                                                                     
    ...
    ERP = pop_binoperator( ERP, {'b19 = ((b1 - b10)/2) Label Odd Incorrect Wave - Even Incorrect Wave', 'b20 = ((b2 - b11)/2) Label Odd Correct Wave - Even Correct Wave', 'b21 = ((b7 - b16)/2) Label Odd Diff Wave - Even Diff Wave' });
    ERP = pop_savemyerp(ERP, 'erpname', [SUB{i} '_ERN_erp_ar_diff_waves_Odd_minus_Even'], 'filename', [SUB{i} '_ERN_erp_ar_diff_waves_Odd_minus_Even.erp'], 'filepath', [DIR filesep SUB{i} filesep]);   
    ...
    ERP = pop_savemyerp(ERP, 'erpname', [SUB{i} '_ERN_erp_ar_diff_waves_Odd_minus_Even_lpfilt'], 'filename', [SUB{i} '_ERN_erp_ar_diff_waves_Odd_minus_Even_lpfilt.erp'], 'filepath', Subject_Path, 'Warning', 'off');
...
for i = 1:length(SUB)  
    ...
    [ERP ALLERP] = pop_loaderp( 'filename', [SUB{i} '_ERN_erp_ar_diff_waves_Odd_minus_Even_lpfilt.erp'], 'filepath', [DIR filesep SUB{i} filesep]);      
end

%Append low-pass filtered ERP waveforms from all subjects 
ERP = pop_appenderp( ALLERP , 'Erpsets', 1:length(SUB), 'Prefixes', SUB);
ERP = pop_savemyerp(ERP, 'erpname', 'All_Sub_Appended_Odd_minus_Even_ERN_lpfilt', 'filename', 'All_Sub_Appended_Odd_minus_Even_ERN_lpfilt.erp', 'filepath', Current_File_Path, 'Warning', 'off');
...
%Plot individual subject plus-minus waveforms for incorrect trials (bin 19)
bin = 19;
...
save2pdf([Current_File_Path filesep 'ERN_PlusMinus_Waveforms_Incorrect.pdf']);
close all

%Plot individual subject plus-minus waveforms for correct trials (bin 20)
bin = 20;
ERP = pop_ploterps( ERP,  bin:21:ERP.nbin,  chan , 'Box', [ 1 1], 'Maximize', 'on', 'Style', 'Classic', 'xscale', xscale, 'yscale', yscale, 'linespec', linecolors);
save2pdf([Current_File_Path filesep 'ERN_PlusMinus_Waveforms_Correct.pdf']);
close all

%Plot individual subject plus-minus waveforms for incorrect minus correct trials (bin 21)
bin = 21;
ERP = pop_ploterps( ERP,  bin:21:ERP.nbin,  chan , 'Box', [ 1 1], 'Maximize', 'on', 'Style', 'Classic', 'xscale', xscale, 'yscale', yscale, 'linespec', linecolors);
save2pdf([Current_File_Path filesep 'ERN_PlusMinus_Waveforms_Incorrect-Correct.pdf']);
close all
...
save2pdf([Current_File_Path filesep 'ERN_PlusMinus_SD.pdf']);
close all;

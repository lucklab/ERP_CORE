...
for i = 1:length(SUB)
    ...
    ERP = pop_savemyerp( ERP, 'erpname', [SUB{i} '_ERN_erp_ar'], 'filename', [Subject_Path SUB{i} '_ERN_erp_ar.erp']);
    ...
    ERP = pop_savemyerp( ERP, 'erpname', [SUB{i} '_ERN_erp_ar_lpfilt'], 'filename', [Subject_Path SUB{i} '_ERN_erp_ar_lpfilt.erp']);
    ...
    fid = fopen([DIR filesep SUB{i} filesep SUB{i} '_AR_Percentages_ERN.csv'], 'w');
...
for i = 1:length(SUB)
    ...
    ERP = pop_loaderp('filename', [SUB{i} '_ERN_erp_ar.erp'], 'filepath', Subject_Path);                                                                                                                                                                                                                                                                                                                                       

    %Create ERP difference waveforms between conditions
    ERP = pop_binoperator( ERP, [Current_File_Path filesep 'ERN_Diff_Wave.txt']);
    ERP = pop_savemyerp(ERP, 'erpname', [SUB{i} '_ERN_erp_ar_diff_waves'], 'filename', [Subject_Path SUB{i} '_ERN_erp_ar_diff_waves.erp']);
    ...
    ERP = pop_filterp( ERP,  1:35 , 'Cutoff',  20, 'Design', 'butter', 'Filter', 'lowpass', 'Order',  8 );
    ERP = pop_savemyerp( ERP, 'erpname', [SUB{i} '_ERN_erp_ar_diff_waves_lpfilt'], 'filename', [Subject_Path SUB{i} '_ERN_erp_ar_diff_waves_lpfilt.erp']);

...
%Current_File_Path = /Users/KappenmanLab/ERP_CORE/ERN/EEG_ERP_Processing/ERP_Measurements
...
%Set measurement time window for measuring mean amplitude, peak amplitude, peak latency, and 50% area latency in milliseconds (e.g., 0 to 100 ms)
timewindow = [0 100];

%Set measurement time window for measuring onset latency (50% peak latency) in milliseconds (e.g., -100 to 100 ms)
timewindow_onsetlat = [-100 100];
...
ERPset_list = fullfile(Current_File_Path, 'Measurement_ERP_List_ERN.txt');
fid = fopen(ERPset_list, 'w');
    for i = 1:length(SUB)
        Subject_Path = [DIR filesep SUB{i} filesep];
        erppath = [Subject_Path SUB{i} '_ERN_erp_ar_diff_waves.erp'];
        fprintf(fid,'%s\n', erppath);
    end
fclose(fid);
...
ALLERP = pop_geterpvalues( ERPset_list, timewindow, diffbin, chan, 'Baseline', baselinecorr, 'Measure', 'meanbl',... 
    'Filename', [Current_File_Path filesep 'Mean_Amplitude_Diff_Waves_ERN.txt'], 'Binlabel', 'on', 'FileFormat', 'wide',... 
...
ALLERP = pop_geterpvalues( ERPset_list, timewindow, diffbin, chan, 'Baseline', baselinecorr, 'Measure', 'fareanlat',... 
    'Afraction', 0.5, 'PeakOnset',  1, 'Fracreplace', 'NaN', 'Filename', [Current_File_Path filesep '50%_Area_Latency_Diff_Waves_ERN.txt'],...
...
ERPset_list = fullfile(Current_File_Path, 'Measurement_ERP_List_lpfilt_ERN.txt');
fid = fopen(ERPset_list, 'w');
    for i = 1:length(SUB)
        Subject_Path = [DIR filesep SUB{i} filesep];
        erppath = [Subject_Path SUB{i} '_ERN_erp_ar_diff_waves_lpfilt.erp'];
        fprintf(fid,'%s\n', erppath);
    end
fclose(fid);
...
ALLERP = pop_geterpvalues( ERPset_list, timewindow, diffbin, chan, 'Baseline', baselinecorr, 'Measure','peakampbl',... 
    ...
    [Current_File_Path filesep 'Peak_Amplitude_Diff_Waves_ERN.txt'], 'Binlabel', 'on', 'FileFormat', 'wide', 'InterpFactor',  1,  'Resolution', 3);
...
ALLERP = pop_geterpvalues( ERPset_list, timewindow, diffbin, chan, 'Baseline', baselinecorr, 'Measure','peaklatbl',... 
    'Peakpolarity', 'negative', 'Neighborhood',  3, 'PeakReplace', 'absolute', 'Filename',... 
    [Current_File_Path filesep 'Peak_Latency_Diff_Waves_ERN.txt'], 'Binlabel', 'on', 'FileFormat', 'wide', 'InterpFactor',  1,  'Resolution', 3);
...
ALLERP = pop_geterpvalues( ERPset_list, timewindow_onsetlat, diffbin, chan, 'Baseline', baselinecorr, 'Measure','fpeaklat', 'Neighborhood',  3,... 
    ...
    [Current_File_Path filesep 'Onset_Latency_Diff_Waves_ERN.txt'], 'Binlabel', 'on', 'FileFormat', 'wide', 'InterpFactor',  1,  'Resolution', 3);
...
ALLERP = pop_geterpvalues( ERPset_list, timewindow, parentbins, chan, 'Baseline', baselinecorr, 'Measure', 'meanbl', 'Filename',... 
    [Current_File_Path filesep 'Mean_Amplitude_Parent_Waves_ERN.txt'], 'Binlabel', 'on', 'FileFormat', 'wide', 'InterpFactor',  1,  'Resolution', 3);

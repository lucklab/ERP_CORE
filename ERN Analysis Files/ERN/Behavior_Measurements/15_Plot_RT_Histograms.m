...
%This script loads the individual subject semi-continuous ICA-corrected EEG data file with an event list from Script #5 (prior to rejecting artifacts), assigns events to correct and incorrect trial bins without applying a reaction time filter using Binlister, 
...
for i = 1:length(SUB)
    ...
    EEG = pop_loadset( 'filename', [SUB{i} '_ERN_shifted_ds_reref_ucbip_hpfilt_ica_corr_cbip_elist.set'], 'filepath', Subject_Path);
 
    %Assign events to incorrect and correct trial bins using no reaction time filter with Binlister (bin assignments can be reviewed in each subject's ERN_Eventlist_For_Histo_RTs_Bins.txt file)
    EEG  = pop_binlister( EEG , 'BDF', [Current_File_Path filesep 'BDF_Histo_RTs_ERN.txt'], 'ExportEL', [Subject_Path SUB{i} '_ERN_Eventlist_For_Histo_RTs_Bins.txt'], 'IndexEL',  1, 'SendEL2', 'EEG&Text', 'UpdateEEG', 'on', 'Voutput', 'EEG' );

    %Extract reaction time (RT) data for each trial per bin and save to a text file in each subject's folder
    pop_rt2text(EEG, 'filename', [Subject_Path SUB{i} '_ERN_Histo_RTs.txt'], 'Header', 'on', 'listformat', 'basic');
...
for i = 1:length(SUB)
    ...
    Single_Sub_Data = readtable([Subject_Path SUB{i} '_ERN_Histo_RTs.txt'],'ReadVariableNames',1,'Delimiter',{'\t'});
    
    Single_Sub_Incorrect_RTs = Single_Sub_Data{:,1};
    Single_Sub_Correct_RTs = Single_Sub_Data{:,2};
    
    %Combine reaction times from all subjects
    All_Sub_Incorrect_RTs = [All_Sub_Incorrect_RTs; Single_Sub_Incorrect_RTs];
    All_Sub_Correct_RTs = [All_Sub_Correct_RTs; Single_Sub_Correct_RTs];
...
Probability_Incorr_Per_RT_Bin = histcounts(All_Sub_Incorrect_RTs,xrange,'Normalization','probability');
Probability_Corr_Per_RT_Bin = histcounts(All_Sub_Correct_RTs,xrange,'Normalization','probability');

bar(bincenters, Probability_Incorr_Per_RT_Bin, w1, 'FaceColor', [0.2 0.2 0.5]);
hold on;
bar(bincenters, Probability_Corr_Per_RT_Bin, w2, 'FaceColor',[0 0.7 0.7]);
hold off;
xticks(bincenters(1:2:end)); 
xlim([xrange(1) xrange(end)]);
ylabel('Probability')
xlabel('RT bins (ms)')
legend({'Incorrects','Corrects'})
set(gca,'fontsize',12);

%Save the histograms to a pdf file
save2pdf([Current_File_Path filesep 'RT_Probability_Histogram_Incorrects_&_Corrects.pdf']);
close all

...
yscale_EEG_parent = [-20.0 30.0   -20:10:30];
...
for i = 1:length(SUB)
    ...
    ERP = pop_loaderp('filename', [SUB{i} '_N400_erp_ar_diff_waves_lpfilt.erp'], 'filepath', Subject_Path);    
    
    %Plot the N400 urelated and related word pairs parent waveforms at the key electrode sites of interest (FCz, Cz, CPz, Pz)
    ...
    save2pdf([Subject_Path 'graphs' filesep SUB{i} '_N400_Parent_Waves.pdf']);
    close all

    %Plot the N400 unrelated-minus-related word pairs difference waveform at the key electrode sites of interest (FCz, Cz, CPz, Pz)
    ...
    save2pdf([Subject_Path 'graphs' filesep SUB{i} '_N400_Difference_Wave.pdf']);
    close all

    %Plot the N400 unrelated and related word pairs parent waveforms at all electrode sites
    ERP = pop_ploterps( ERP, [1 2], [1:35] , 'Box', [6 7], 'blc', baselinecorr, 'Maximize', 'on', 'Style', 'Classic', 'xscale', xscale,  'yscale', yscale_EEG_parent);
    save2pdf([Subject_Path 'graphs' filesep SUB{i} '_N400_Parent_Waves_All_Channels.pdf']);
    close all

    %Plot the N400 unrelated-minus-related word pairs difference waveform at all electrode sites
    ERP = pop_ploterps( ERP, [3], [1:35] , 'Box', [6 7], 'blc', baselinecorr, 'Maximize', 'on', 'Style', 'Classic', 'xscale', xscale,  'yscale', yscale_EEG_diff);
    save2pdf([Subject_Path 'graphs' filesep SUB{i} '_N400_Difference_Wave_All_Channels.pdf']);
    close all
  
    %Plot the parent (unrelated and related word pairs conditions) ICA-corrected and uncorrected bipolar HEOG signals 
    ERP = pop_ploterps( ERP, [1 2], [32 34] , 'Box', [1 2], 'blc', baselinecorr, 'Maximize', 'on', 'Style', 'Classic', 'xscale', xscale,  'yscale', yscale_HEOG);
    save2pdf([Subject_Path 'graphs' filesep SUB{i} '_N400_HEOG.pdf']);
    close all
    
    %Plot the parent (unrelated and related word pairs conditions) ICA-corrected monopolar VEOG signals and corrected bipolar VEOG signal
    ERP = pop_ploterps( ERP, [1 2], [15 31 33] , 'Box', [2 2], 'blc', baselinecorr, 'Maximize', 'on', 'Style', 'Classic', 'xscale', xscale,  'yscale', yscale_VEOG);
    save2pdf([Subject_Path 'graphs' filesep SUB{i} '_N400_VEOG.pdf']);
    close all

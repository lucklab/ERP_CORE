...
fid = fopen(fullfile(Current_File_Path, 'MeasurementWindow_Noise_N400.csv'), 'w');
fprintf(fid, 'SubID, Unrelated, Related, Unrelated-Related, Unrelated Filtered, Related Filtered, Unrelated-Related Filtered\n');

%***********************************************************************************************************************************************

%Set measurement time window in milliseconds (e.g., 300 to 500 ms)
timewindow = [300 500];
...
for i = 1:length(SUB)
    ...
    ERP = pop_loaderp('filename', [SUB{i} '_N400_erp_ar_diff_waves_Odd_minus_Even.erp'], 'filepath', Subject_Path); 
    ...
    ERP = pop_loaderp('filename', [SUB{i} '_N400_erp_ar_diff_waves_Odd_minus_Even_lpfilt.erp'], 'filepath', Subject_Path); 
...
meas_noise_table = readtable([Current_File_Path filesep 'MeasurementWindow_Noise_N400.csv']);
...
legend({'Unrelated','Related','Unrelated minus Related'});
set(gca,'fontsize',12);

%Save measurement window noise plots to pdf
save2pdf([Current_File_Path filesep 'N400_MeasurementWindow_Noise_Unfiltered.pdf']);
close all
...
%Save measurement window noise plots to pdf
save2pdf([Current_File_Path filesep 'N400_MeasurementWindow_Noise_Filtered.pdf']);
close all

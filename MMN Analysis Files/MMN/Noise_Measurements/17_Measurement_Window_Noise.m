...
%Uses the output from Script #16: PlusMinus_Averages.m
...
fid = fopen(fullfile(Current_File_Path, 'MeasurementWindow_Noise_MMN.csv'), 'w');
fprintf(fid, 'SubID, Deviants, Standards, Deviants-Standards, Deviants Filtered, Standards Filtered, Deviants-Standards Filtered\n');

%***********************************************************************************************************************************************

%Set measurement time window in milliseconds (e.g., 125 to 225 ms)
timewindow = [125 225];
...
for i = 1:length(SUB)
    ...
    ERP = pop_loaderp('filename', [SUB{i} '_MMN_erp_ar_diff_waves_Odd_minus_Even.erp'], 'filepath', Subject_Path); 
    ...
    ERP = pop_loaderp('filename', [SUB{i} '_MMN_erp_ar_diff_waves_Odd_minus_Even_lpfilt.erp'], 'filepath', Subject_Path); 
...
meas_noise_table = readtable([Current_File_Path filesep 'MeasurementWindow_Noise_MMN.csv']);
...
legend({'Deviants','Standards','Deviants minus Standards'});
set(gca,'fontsize',12);

%Save measurement window noise plots to pdf
save2pdf([Current_File_Path filesep 'MMN_MeasurementWindow_Noise_Unfiltered.pdf']);
close all
...
%Save measurement window noise plots to pdf
save2pdf([Current_File_Path filesep 'MMN_MeasurementWindow_Noise_Filtered.pdf']);
close all

...
fid = fopen(fullfile(Current_File_Path, 'Baseline_Noise_MMN.csv'), 'w');
...
for i = 1:length(SUB)
    ...
    ERP = pop_loaderp('filename', [SUB{i} '_MMN_erp_ar_diff_waves.erp'], 'filepath', Subject_Path); 
...
baseline_noise_table = readtable([Current_File_Path filesep 'Baseline_Noise_MMN.csv']); 
...
save2pdf([Current_File_Path filesep 'MMN_Baseline_Noise_Unfiltered.pdf']);
close all
...
%Save baseline noise plots to pdf
save2pdf([Current_File_Path filesep 'MMN_Baseline_Noise_Filtered.pdf']);
close all

...
fid = fopen(fullfile(Current_File_Path, 'MeasurementWindow_Noise_N2pc.csv'), 'w');
fprintf(fid, 'SubID, Contra, Ipsi, Contra-Ipsi, Contra Filtered, Ipsi Filtered, Contra-Ipsi Filtered\n');

%***********************************************************************************************************************************************

%Set measurement time window in milliseconds (e.g., 200 to 275 ms)
timewindow = [200 275];
...
for i = 1:length(SUB)
    ...
    ERP = pop_loaderp('filename', [SUB{i} '_N2pc_erp_ar_diff_waves_Odd_minus_Even.erp'], 'filepath', Subject_Path); 
    ...
    ERP = pop_loaderp('filename', [SUB{i} '_N2pc_erp_ar_diff_waves_Odd_minus_Even_lpfilt.erp'], 'filepath', Subject_Path); 
...
meas_noise_table = readtable([Current_File_Path filesep 'MeasurementWindow_Noise_N2pc.csv']);
...
save2pdf([Current_File_Path filesep 'N2pc_MeasurementWindow_Noise_Unfiltered.pdf']);
close all
...
%Save measurement window noise plots to pdf
save2pdf([Current_File_Path filesep 'N2pc_MeasurementWindow_Noise_Filtered.pdf']);
close all

...
for i = 1:length(SUB)
    ...
    [ALLEEG EEG CURRENTSET] = pop_newset(ALLEEG, EEG, 1, 'setname', [SUB{i} '_N170_shifted_ds_reref_ucbip_hpfilt'], 'gui', 'off'); 
    ...
    EEG  = pop_erplabDeleteTimeSegments( EEG , 'displayEEG', 0, 'endEventcodeBufferMS',  2000, 'ignoreUseEventcodes', [1:80 100:180], 'ignoreUseType', 'Use', 'startEventcodeBufferMS',  2000, 'timeThresholdMS',  2000 );
    [ALLEEG EEG CURRENTSET] = pop_newset(ALLEEG, EEG, 2, 'setname', [SUB{i} '_N170_shifted_ds_reref_ucbip_hpfilt_ica_prep1'], 'savenew', [Subject_Path SUB{i} '_N170_shifted_ds_reref_ucbip_hpfilt_ica_prep1.set'], 'gui', 'off'); 

    %Load parameters for rejecting especially noisy segments of EEG during trial blocks from Excel file ICA_Prep_Values_N170.xls. Default parameters can be used initially but may need 
    % to be modified for a given participant on the basis of visual inspection of the data.
    [ndata, text, alldata] = xlsread([Current_File_Path filesep 'ICA_Prep_Values_N170']); 
    ...
    [ALLEEG EEG CURRENTSET] = pop_newset(ALLEEG, EEG, 3, 'setname', [SUB{i} '_N170_shifted_ds_reref_ucbip_hpfilt_ica_prep2'], 'savenew', [Subject_Path SUB{i} '_N170_shifted_ds_reref_ucbip_hpfilt_ica_prep2.set'], 'gui', 'off'); 

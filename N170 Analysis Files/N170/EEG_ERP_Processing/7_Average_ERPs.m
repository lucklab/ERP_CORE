...
for i = 1:length(SUB)
    ...
    ERP = pop_savemyerp( ERP, 'erpname', [SUB{i} '_N170_erp_ar'], 'filename', [Subject_Path SUB{i} '_N170_erp_ar.erp']);
    ...
    ERP = pop_savemyerp( ERP, 'erpname', [SUB{i} '_N170_erp_ar_lpfilt'], 'filename', [Subject_Path SUB{i} '_N170_erp_ar_lpfilt.erp']);
    ...
    %Calculate the total percentage of trials rejected across all trial types (first four bins)
    total_accepted = accepted(1) + accepted(2) + accepted(3) + accepted(4);
    total_rejected= rejected(1)+ rejected(2) + rejected(3)+ rejected(4);
    ...
    fid = fopen([DIR filesep SUB{i} filesep SUB{i} '_AR_Percentages_N170.csv'], 'w');
...
for i = 1:length(SUB)
    ...
    ERP = pop_loaderp('filename', [SUB{i} '_N170_erp_ar.erp'], 'filepath', Subject_Path);                                                                                                                                                                                                                                                                                                                                       

    %Create ERP difference waveforms between conditions
    ERP = pop_binoperator( ERP, [Current_File_Path filesep 'N170_Diff_Wave.txt']);
    ERP = pop_savemyerp(ERP, 'erpname', [SUB{i} '_N170_erp_ar_diff_waves'], 'filename', [Subject_Path SUB{i} '_N170_erp_ar_diff_waves.erp']);
    ...
    ERP = pop_filterp( ERP,  1:35 , 'Cutoff',  20, 'Design', 'butter', 'Filter', 'lowpass', 'Order',  8 );
    ERP = pop_savemyerp( ERP, 'erpname', [SUB{i} '_N170_erp_ar_diff_waves_lpfilt'], 'filename', [Subject_Path SUB{i} '_N170_erp_ar_diff_waves_lpfilt.erp']);
